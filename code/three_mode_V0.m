...
tic;
%加载Hon-------------------(-0.10,0.10)
load('.\config\Hon\fM.mat');
Hon_=fM;
clear fM;

%加载Dim-------------------(0.05,0.10)
load('.\config\Dim\fM.mat');
Dim_=fM;
clear fM;

%加载Rec-------------------(0.00,-0.05)
load('.\config\Rec\fM.mat');
Rec_=fM;
clear fM;

%加载Kag-------------------(0.10,0.00)
load('.\config\Kag\fM.mat');
Kag_=fM;
clear fM;

%加载Tri-------------------(-0.40,-0.05)
load('.\config\Tri\fM.mat');
Tri_=fM;
clear fM;
...
b1=0;  b2=0.05; b3=0.1;            %权重
co=Rec_;                          %放置晶格
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
a=0;                              %立方项系数
V0=0;                             %薄膜-基底势能
size=100;                          %放置晶体大小,注意要小于Nx,且必须是偶数
