...
load('C:\Users\反冲加速\Desktop\毕业设计\毕设论文与代码\PFC_code\pfc_V0\三模data\石墨烯\gra512.mat');
ge=fM;
for i=1:Nx
    for j=1:Ny
V(i,j)=V0*fM(i,j);
...
%初始化原子密度
    for i=1:Nx
      for j=1:Ny
       %   if((i>(Nx/2-200))&&(i<(Nx/2+200))&&(j<(Nx/2+200))&&(j>(Nx/2-200))) den(i,j)=ge(i,j);
...
energy=sum(ff(:))/(Nx*Ny);      %对每个能量矩阵元素求和
